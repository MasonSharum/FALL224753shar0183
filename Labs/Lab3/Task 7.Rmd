...
inputPanel(
  ...
  selectInput("plotg", "Choose plot type",
      list("Linear Regression Line", "Points", "Segments"
      )
    )
)
...
renderPlot({
 ...
 Point = ggplot(spruce.df, aes(x = BHDiameter, y = Height)) + geom_point() 

 Linear = Point + geom_smooth(method = "lm", level = 0.00)
 
 Segment = Point + geom_line() 
 

 
  
 if(input$plotg == "Linear Regression Line") print(Linear)
 
 if(input$plotg == "Points")   print(Point)
 
 if(input$plotg == "Segments")   print(Segment)
